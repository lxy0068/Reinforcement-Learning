...
def plot_values(values, xlabel, ylabel, title=None):
    """
...
plot_values(train_scores, "Episodes", "Scores", title="Training Scores")

mean_scores = [np.mean(train_scores[max(0, i - 99):i + 1]) for i in range(len(train_scores))]
plot_values(mean_scores, "Episodes", "Mean Scores", title="Mean Scores Over Last 100 Episodes")

plot_values(epsilons, "Episodes", "Epsilon", title="Epsilon Decay Over Episodes")

test_scores = test("dqn_model.pth")

plot_values(test_scores, "Episodes", "Scores", title="Test Scores")
